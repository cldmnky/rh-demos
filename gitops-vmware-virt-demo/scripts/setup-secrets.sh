...
echo "  2. Allow ArgoCD Applications in this namespace:"
echo "     oc patch argocd openshift-gitops -n openshift-gitops --type=merge -p '{\"spec\":{\"sourceNamespaces\":[\"$NAMESPACE\"]}}'"
echo "  3. Apply ArgoCD resources:"
echo "     oc apply -f argocd/appproject.yaml"
echo "     oc apply -f argocd/application.yaml -n $NAMESPACE"
echo "     oc apply -f argocd/application-infra.yaml -n $NAMESPACE"
echo "  4. Trigger install: oc create -f pipelines/install-pipelinerun.yaml -n $NAMESPACE"
