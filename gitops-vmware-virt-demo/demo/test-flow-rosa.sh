...
#   cd gitops-vmware-virt-demo && ./demo/test-flow-rosa.sh
...
sync_argo_git() {
  ...
  run oc patch application.argoproj.io "${app}" -n "${NAMESPACE}" \
    --type merge \
    --patch "{\"operation\":{\"initiatedBy\":{\"username\":\"test-flow\"},\"sync\":{\"revision\":\"${revision}\",\"prune\":true}}}"
...
wait_for_lb_ip() {
  ...
  log "Waiting for AWS LoadBalancer hostname/IP"
...
verify_rosa_prereqs() {
  ...
  log "Verifying ROSA prerequisites"
  run oc get clusterversion version
  run oc get hyperconverged kubevirt-hyperconverged -n openshift-cnv
  run oc get storageprofile "${ROSA_STORAGE_CLASS}"
  run oc get volumesnapshotclass csi-trident-vsc
  ...
  [[ "${current_snapshot}" == "${DEFAULT_GREEN_SNAPSHOT_NAME}" && "${snapshot_ns}" == "${DEFAULT_GREEN_SNAPSHOT_NS}" ]] || {
...
log "Using AWS native LoadBalancer"
...
run oc apply -f "${DEMO_ROOT}/argocd/application-rosa.yaml" -n "${NAMESPACE}"
...
UPGRADE_PR=$(oc create -f "${DEMO_ROOT}/pipelines/upgrade-pipelinerun-rosa.yaml" -n "${NAMESPACE}" -o name)
